    ...
    <div Align="right">
        <table class="table  tabel-bordered  table-striped">
            ...
            <td>{{ $row['id']}}</td>
                ...
                <td>
                    <a href="{{action('AgendasContronller@edit', $row['id']) }}" class="btn btn-warning"><i class="fa fa-wrench"></i></a>
                </td>
                <td>
                    <form method="post" class="delete_form" action="{{ action('AgendasContronller@destroy', $row['id']) }}">
                        {{ csrf_field() }}
